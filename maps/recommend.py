...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min([centroid for centroid in centroids], key = lambda centroid: distance(location,centroid))
    # END Question 3
...
def find_predictor(user, restaurants, feature_fn):
    ...
    xy = zip(xs, ys)
    Sxy = sum([(point[0] - mean(xs))* (point[1] - mean(ys)) for point in xy])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return max([find_predictor(user, reviewed, function) for function in feature_fns], key = lambda tuple: tuple[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    def which_rating(restaurant):
        if restaurant in reviewed:
            return user_rating(user, restaurant_name(restaurant))
        else:
            return predictor(restaurant)
    return {restaurant_name(restaurant): which_rating(restaurant) for restaurant in restaurants}
